...
function tvpreview_set_channel (index, episode_id, reportflag)
  {
log ("QQ TVPREVIEW_SET_CHANNEL"); state();
  ipg_cursor = index;
...
function tvpreview_set_channel_inner (real_channel, episode_id, reportflag)
  {
log ("QQ TVPREVIEW_SET_CHANNEL_INNER"); state();
  var channel = pool [real_channel];
...
function tvpreview_yt_init()
  {
log ("QQ TVPREVIEW_YT_INIT"); state();
...
function tvpreview_play_yt()
  {
log ("QQ TVPREVIEW_PLAY_YT"); state();
  if (thumbing == 'ipg')
    return;
...
function play_this_youtube_video (pid)
  {
log ("QQ PLAY THIS YOUTUBE VIDEO"); state();
...
function preload_youtube_videos (use_current_video_flag)
  {
  if (preload == 'off')
    return;

  var next_program = parseInt (program_cursor) + 1;
  if (next_program > n_program_line || use_current_video_flag)
    next_program = 1;
...
function switch_to_ipg_inner()
  ...
  for (var i = 1; i <= 10; i++)
    setTimeout ("delayed_pause_video()", i * 500);

  tvpreview_yt_init()
  set_preload_timer();
  }

function set_crazy_z_indexes()
  { 
return;
  $("#bg-layer").css  ("z-index", "79");
  $("#sg-grid").css   ("z-index", "81");
  $("#sg-header").css ("z-index", "81");
  $("#sg-holder").css ("z-index", "80");
  $("#sg-layer").css  ("z-index", "80");
  }

function unset_crazy_z_indexes()
  {
return;
  $("#bg-layer").css  ("z-index", "10");
  $("#sg-grid").css   ("z-index", "51");
  $("#sg-header").css ("z-index", "51");
  $("#sg-holder").css ("z-index", "50");
  $("#sg-layer").css  ("z-index", "50");
  }

var preload_timex;

function set_preload_timer()
  {
  clearTimeout (preload_timex);
  preload_timex = setTimeout ('set_preload_timer_inner("' + ipg_cursor + '")', 1000);
  }

function set_preload_timer_inner (cursor)
  {
  if (thumbing == 'ipg' && cursor == ipg_cursor)
    {
    log ('preload timer ding');
    preload_current_square();
    }
  }

function preload_current_square()
  {
  var channel = channelgrid [ipg_cursor];
  program_lineup (channel ['id']);
  if (channel ['youtubed'])
    ...
    if (n_program_line > 0)
      {
      /* preload right now */ ;
      log ('preload right now');
      preload_current_square_inner (ipg_cursor);
      }
    else
      log ('no episodes to preload');
    }
  else
    {
    /* must first load channel */
    var nature = channel ['nature'];
    if (nature == '3' || nature == '4' || nature == '5')
      fetch_youtube_or_facebook (channel ['id'], 'preload_current_square_inner("' + ipg_cursor + '")');
    else if (nature == '1' || nature == '6' || nature == '7' || nature == '8' || nature == '9')
      standard_channel (channel ['id'], 'preload_current_square_inner("' + ipg_cursor + '")');
    }
  }

function preload_current_square_inner (cursor)
  {
  return;
  if (cursor == ipg_cursor)
    ...
    $("#player-layer").show();
    low_video();
    preload_youtube_videos (true);
    }
  else
    log ("preload_current_square_inner: we have moved on, won't preload");
  }
...
function which_relay()
  ...
  if (location.host == 'www.9x9.tv' || location.host == '9x9.tv' || location.host == 'stage.9x9.tv')
    which = "relay-prod.9x9.tv";
  else if (location.host == 'v31.9x9.tv')
    which = "relay-v31.9x9.tv";
...
function state()
  ...
  for (var i = 1; i <= 3; i++)
    ...
    if (video_id.length == 11 && yt_state >= 1)
      ytmini_video_id [i] = video_id;
    else
      ytmini_video_id [i] = undefined;
    }
