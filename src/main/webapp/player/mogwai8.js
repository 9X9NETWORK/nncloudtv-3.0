  ...
  if (lines.length > 0)
    ...
    for (var i = 2; i < lines.length; i++)
      ...
      if (lines [i] != '')
        ...
        var pinfo = line_to_program (lines[i]);
        ...
        if (pinfo ['url1'].match (/\|/))
          ...
          for (var j = 1; j < urls.length; j++)
            ...
            if ((j+1) in begin_titles)
              pcopy ['begin-title'] = begin_titles [j];
            if ((j+1) in end_titles)
              pcopy ['end-title'] = end_titles [j];
            pcopy ['umbrella'] = pcopy ['id'];
            pcopy ['umbrella-name'] = names [0];
            pcopy ['umbrella-desc'] = descs [0];
            pcopy ['umbrella-thumb'] = thumbs [0];
            pcopy ['proto'] = pinfo;
            programgrid [pcopy ['id'] + '-' + j] = pcopy;
            }
          }
        else
          {
...
function line_to_program (line)
  ...
  var pinfo = { 'id': fields[1], 'channel': fields[0], 'type': fields[3], 'url1': 'fp:' + fields[8],
  ...
  if (pinfo ['url4'] && ( !pinfo ['url1'] || pinfo ['url1'] == 'fp:|' ))
    {
...
function redraw_player_column()
  ...
  if (player_mode == 'guide')
    ...
    $("#recommendation .manage-tip").css ("visibility", "hidden");
  else if (player_mode == 'trending')
