...
/* curators known to have custom urls */
var canonicalized_curators = {};
...
var nroot = 'http://9x9ui.s3.amazonaws.com/mock42/images/';
...
function set_language (lang)
  ...
  if (true)
    {
    $("#video-source").html ('<span>' + translations ['videosource'] + '</span>');
    $("#video-source").unbind();
    $("#video-source").click (visit_youtube_episode_externally);
    $("#curator-source").html ("");
    }
  else if (false)
    {
...
function hash_changed()
  ...
  if (location.hash != current_hash)
    ...
    hash_changed_inner (location.hash);
    }
  }

function hash_changed_inner (current_hash)
  {
...
function which_relay()
  ...
  if (location.host == 'www.9x9.tv' || location.host == 'player.9x9.tv' || location.host == '9x9.tv' || location.host == 'stage.9x9.tv' || location.host.match (/v32.9x9.tv/))
...
function init_progress_bar()
  ...
  for (var s in program ['segments'])
    ...
    if (s > 0)
      ...
      html += '<li class="progress-titlecard" style="width: ' + width + '; left: ' + left + '; background-color: ' + bg + '" data-segment="' + s + '"></li>';
  ...
  $("#sub-ep li").hover (function()
    {
    log ("SUB EP POINT HOVER IN: " + $(this).attr("id"));
  ...
  function()
    {
    log ("SUB EP POINT HOVER OUT: " + $(this).attr("id"));
    });
  $("#sub-ep li").click (function()
    {
    log ("SUB EP POINT CLICK: " + $(this).attr("id"));
    var id = $(this).attr ("id").replace (/^clip-/, '');
    var segments = get_current_program()['segments'];
    var new_segment = parseInt (id) - 1;
...
function parse_program_line (count, line)
  ...
  if (pinfo ['url1'].match (/\|/))
    ...
    pinfo ['duration'] = colonic_time_to_seconds (durs [0]);
    pinfo ['type'] = types [0];
    ...
    for (var j = 1; j < urls.length; j++)
      ...
      var dur = durs ? parseInt (durs [j].replace (/;0;/, '')) : 60;
      var start_time;
      var end_time;
      var clipped = false;
      dur = colonic_time_to_seconds (dur);
      if (urls [j].match (/;(\d+);(\d+)$/))
        {
        /* this video is clipped -- it has start and end times set */
        clipped = true;
        start_time = parseInt (urls [j].match (/;(\d+);(\d+)$/) [1]);
        end_time = parseInt (urls [j].match (/;(\d+);(\d+)$/) [2]);
        if (end_time - start_time != dur)
          {
          log ("duration fixed up: " + dur + " -> " + (end_time - start_time));
          dur = end_time - start_time;
          }
        if (false && start_time == 0 && end_time == dur)
          {
          /* someone wants the entire range of clip. but this may differ from true length of YouTube video */
          clipped = false;
      ...
      var sub = { url: url1, duration: dur, name: names [j], desc: descs [j], thumb: thumbs [j], type: types [j], snapshot: snapshots [j], clipped: clipped, 'start-time': start_time, 'end-time': end_time };
    ...
    {
    var dur = colonic_time_to_seconds (pinfo ['duration']);
    /* this shouldn't happen any more, there should always be a | in the URL. but it does happen. */
    var sub = { url: pinfo ['url1'], duration: dur, name: pinfo ['name'], desc: pinfo ['desc'], thumb: pinfo ['thumb'], type: pinfo ['type'], snapshot: pinfo ['snapshot'] };
...
log ('line_to_program: id=' + fields[1] + ' channel=' + fields[0]);
log ('submeta: ' + fields[14]);
 
  var submeta = ""; 
  try { submeta = decodeURIComponent (fields[14]); } catch (error) {};

  if (fields[5].match (/:/) && ! (fields[5].match (/\|/)))
    fields[5] = colonic_time_to_seconds (fields[5]);
 
  var pinfo = { 'id': fields[1], 'channel': fields[0], 'url1': fields[8],
                'url2': fields[9], 'url3': fields[10], 'url4': fields[11],
                 ...
                 'submeta': submeta, 'duration': fields[5] };
...
function redraw_home_trending (starting_at)
  ...
  $("#trending-stories-right li").click (home_trending_play);
  $("#trending-stories-right li").hover (function()
    {
    home_trending_hover ($(this).attr("id"));
    },
  function()
    {
    });
  $("#trending-stories-right li").click (function()
    {
    });
  }

function home_trending_hover (id)
  {
...
function home_trending_inner (id)
  ...
  $("#trending-stories .trending-box").mouseover (function() { clearTimeout (home_timex); });
  $("#trending-stories .trending-box").mouseout (function() { start_home_autorotation(); });
...
function custom_url (path)
  ...
  if ((location.hash).match (/\!landing=9x9$/))
    ...
    set_hash ('');
    var fb_return_hash = getcookie ("fb-return-hash");
    var fb_return_subscribe = getcookie ("fb-return-subscribe");
    if (fb_return_hash)
      {
      log ("FB RETURN HASH: " + fb_return_hash + " SUBSCRIBE: " + fb_return_subscribe);
      deletecookie ("fb-return-hash");
      }
    if (fb_return_subscribe)
      {
      deletecookie ("fb-return-subscribe");
      if (fb_return_subscribe in pool)
        {
        log ('[fb login] subscribing to remembered channel: ' + fb_return_subscribe);
        browse_accept (fb_return_subscribe, function() { hash_changed_inner (fb_return_hash); });
        }
      else
        {
        log ('[fb login] want to subscribe to remembered channel: ' + fb_return_subscribe + ', loading it...');
        load_channel_then (fb_return_subscribe, function()
          {
          log ('[fb login] subscribing (inner) to remembered channel: ' + fb_return_subscribe);
          browse_accept (fb_return_subscribe, function() { hash_changed_inner (fb_return_hash); }); 
          });
        }
      }
    else if (fb_return_hash)
      hash_changed_inner (fb_return_hash);
    else
      { /* ordinary facebook login */ }
    return;
    }
  else if ((location.hash).match (/\!share=([^\&=]+)/))
    {
...
function landing_with_hash()
  {
  var id = (location.hash).replace (/^#!/, '');

  log ("LANDING WITH HASH: " + id);
  ...
  var d = $.get (query, function (data)
    ...
    if (fields[0] == '0')
      {
      canonicalized_curators [id] = true;
      curation (id);
      }
    });
  }
...
function view_landing()
  ...
  if (!channel_id && !episode_id && location.hash != '')
    {
    landing_with_hash();
    return;
    }
...
function preload_any_titlecard_graphics()
  {
  var segments = get_current_program()['segments'];
  if (segments)
    {
    for (var s in segments)
      if (s > 0)
        if ('titlecard' in segments [s])
          {
          var titlecard = segments [s]['titlecard'];
          if ('bgimage' in titlecard)
            {
            log ('preloading titlecard graphic: ' + titlecard ['bgimage']);
            var img = new Image();
            img.src = titlecard ['bgimage'];
            }
          }
    }
  }
...
function colonic_time_to_seconds (s)
  {
  var duration = 0;
  if (typeof (s) == 'string' && s.match (/:/))
    ...
    if (fields.length == 3)
      ...
      duration = parseInt (fields[0]) * 60 + parseInt (fields [1]);
    else if (fields.length == 1)
      duration = parseInt (fields [0]);
    }
  else
    duration = Math.floor (s);
  return duration;
  }
...
function recalculate_total_duration()
  ...
  for (var s in segments)
    ...
    if (s > 0)
      {
      var duration = 0;
      var clip = segments [s]['clip'];
      if (segments [s]['type'] != 'video')
        duration = parseInt (segments [s]['duration']);
      else if ('measured-duration' in clip)
        duration = parseFloat (clip ['measured-duration']);
      else if (typeof (get_current_clip()['duration']) == 'string')
        {
        duration = colonic_time_to_seconds (get_current_clip()['duration']);
        }
      else
        duration = parseFloat (clip ['duration'])
      log ("[recalculate] segment " + s + " duration " + duration + " cumulative " + total_duration);
      segments [s]['starting'] = total_duration;
  ...
  if (!program ['duration'])
    program ['duration'] = total_duration;
  }
...
function durationof (duration)
  ...
  if (duration == '' || duration == 'null' || duration == undefined || duration == Infinity || isNaN (duration))
    return '0:00';
    ...
    for (var i in fbfeed['data'])
      ...
      if (link && link.match ('(youtube\.com|youtu\.be)'))
        ...
        programgrid [program_id] = { 'channel': real_channel, 'url1': link, 
                                     ...
                                     'total-duration': -1, segments: segments };
        }
      else if (link)
        {
        /* not a youtube link */
  ...
  if (data && data.feed)
    ...
    for (var i = 0; i < entries.length; i++)
      ...
      programgrid [program_id] = { id: program_id, 'channel': channel ['id'], 'url1': url,
                                   ...
                                   'total-duration': -1, 'sort': i+1, segments: segments };
      }
...
function set_player_ep()
  {
  $("#player-ep-list .clickable").removeClass ("on");
  if (n_program_line > 0)
    {
    $("#p-li-" + program_cursor).addClass ("on");
    var width_per = $("#player-ep-list li").eq(0).width() + parseInt ($("#player-ep-list li").eq(1).css("margin-right"));
    var allowed = Math.floor ($("#player-ep-constrain").width() / width_per);
    var current_starting_index = -1 * parseInt ($("#player-ep-list").css ("margin-left")) / width_per;
    log ("current_starting_index: " + current_starting_index + ", allowed: " + allowed);
    var pc0 = program_cursor - 1; /* program_cursor based at 0 */
    if (pc0 < current_starting_index || pc0 >= current_starting_index + allowed)
      $("#player-ep-list").animate ({ "margin-left": (-1 * pc0 * width_per) + "px" }, 1000);
    }
  }
...
function redraw_program_line()
  ...
  var width_per = $("#player-ep-list li").eq(0).width() + parseInt ($("#player-ep-list li").eq(1).css("margin-right"));
  $("#player-ep-list").css ("width", ((width_per + 10) * n_program_line) + "px");
...
function new_submit_login (callback_on_success)
  ...
  $.post ("/playerAPI/login", serialized, function (data)
    ...
    if (fields [0] == "0")
      ...
      /* the stacks may not be for the correct region yet */
      if (sphere != stack_sphere)
        auxlogin (user);
...
function process_login_data_line (line)
  ...
  else if (fields [0] == 'fbUser')
    is_facebook_user = (fields [1] == '1');
  }
      ...
      for (var i = 2; i < lines.length; i++)
        ...
        if (fields[0] == 'read-only')
          ...
          if (locale == 'en' || locale == 'zh')
            {
            log ("sphere detected as: " + locale);
            sphere = locale;
            // set_language (locale);
            footer_locale();
            }
          }
        else if (fields[0] == 'acceptLang')
          {
          set_language (fields[1].substr(0,2).toLowerCase());
          }
        }
      login();
...
function load_new_sphere_data()
  ...
  var d = $.get ("/playerAPI/category?lang=" + sphere + '&' + 'v=32' + rx(), function (data)
    {
    var lines = data.split ('\n');
    var fields = lines[0].split ('\t');
    if (fields[0] == '0')
...
function set_username()
  ...
  if (username == 'Guest')
    ...
    $("#profile").hide();
    $("#signin").show();
    $("#btn-share, #btn-favorite").hide();
    }
  else
    {
    $("#signin").hide();
    $("#profile").show();
    $("#btn-share, #btn-favorite").show();
    }
  set_username_clicks();
...
function flippr_buttons()
  ...
  $("#flipr-arrowleft").unbind();
  $("#flipr-arrowleft").click (flip_prev_episode);

  $("#btn-ep-left").unbind();
  $("#btn-ep-left").click (flip_back_n);

  $("#flipr-arrowright").unbind();
  $("#flipr-arrowright").click (flip_next_episode);

  $("#btn-ep-right").unbind();
  $("#btn-ep-right").click (flip_ahead_n);
  }
...
function player_sidebar_adjust()
  ...
  // var sh = wh - 130;
  var sh = wh - $("#pl-constrain").offset().top;
  // var bh = wh - 260;
  $("#pl-constrain").css ({ height: sh });
  $("#pl-slider").css ({ height: sh - 10 });
...
function favorite()
  ...
  var deletion = (current_program_is_favorited() 
         || (channel ['curatorid'] == curatorid && (channel ['nature'] == '11' || channel ['nature'] == '12')));
...
function current_program_is_favorited()
  ...
  /* when in favorites channel */
  if (channel ['curatorid'] == curatorid && (channel ['nature'] == '11' || channel ['nature'] == '12'))
    {
    /* Even though it's in favorites channel, it got unfavorited during this session */
    if (user in favorited && player_real_channel in favorited [user])
      return ( favorited [user][player_real_channel][program_id] == true );
    /* Normal case, things in favorites channel are favorites */
    return true;
    }
...
function flip_ahead_n()
  {
  log ('flip ahead n');
  var was = program_cursor;
  var width_per = $("#player-ep-list li").eq(0).width() + parseInt ($("#player-ep-list li").eq(1).css("margin-right"));
  var allowed = Math.floor ($("#player-ep-constrain").width() / width_per);
  if (parseInt (program_cursor) + allowed < n_program_line)
    program_cursor = parseInt (program_cursor) + allowed
  else
    program_cursor = n_program_line;
  log ('flip ahead ' + allowed + ' (' + was + ' -> ' + program_cursor + ')');
  flip_n_inner();
  }

function flip_back_n()
  {
  log ('flip back n');
  var was = program_cursor;
  var width_per = $("#player-ep-list li").eq(0).width() + parseInt ($("#player-ep-list li").eq(1).css("margin-right"));
  var allowed = Math.floor ($("#player-ep-constrain").width() / width_per);
  if (parseInt (program_cursor) - allowed >= 1)
    program_cursor = parseInt (program_cursor) - allowed;
  else
    program_cursor = 1;
  log ('flip back ' + allowed + ' (' + was + ' -> ' + program_cursor + ')');
  flip_n_inner();
  }

function flip_n_inner()
  {
  current_segment = 0;
  redraw_program_line();
  player_metainfo();
...
function freshen_content()
  {
  log ('language changed: now freshen content (thumbing: ' + thumbing + ')');
  var most_recent_channel = player_history_ids [player_history_ids.length - 1];
  /* mrc is used to give the recommended stack guidance in selecting appropriate content */
...
dblocks = blocks;
    ...
    $("#developer-list").css ("top", "0px");
    scrollbar ("#developer-constrain", "#developer-list", "#developer-slider");
    setTimeout ('$("#developer-slider .slider-vertical").slider ("value", "100")', 200);
...
function account_dropdown()
  ...
  var li2 = is_facebook_user ? '' : ('<li id="btn-settings"><span>' + translations ['settings'] + '</span></li>');
...
function settings()
  ...
  var d = $.get ("/playerAPI/getUserProfile?user=" + user + rx(), function (data)
    ...
    $("#btn-change-password").click (function() { settings_change_password (false); });
...
function settings_change_password (reset_flag)
  ...
  if (reset_flag)
    ...
    $("#settings-panel-change li").eq(0).show();
...
var highest_search_channel = 0;
var max_search_channel = 0;
...
function perform_search (text)
  ...
  var d = $.get ("/playerAPI/search?text=" + encodeURIComponent (text) + '&' + "start=1" + '&' + "count=9" + rx(), function (data)
     {
     var html = '';
     log ('result: ' + data);
     var blocks = data.split (/^--$/m);
sblocks = blocks;
     ...
     for (var i = 0; i < lines.length; i++)
       ...
       var fields = lines[i].split ('\t');
       if (fields[0] == 'channel')
         max_search_channel = parseInt (fields[2]);
       }
     ...
     /* CHANNEL RESULTS */
     $("#channel-result").html ("");
     highest_search_channel = 0;
     append_search_channel_result (blocks[4]);

     var cu_plural = (curator_count == 1) ? translations ['ncurator'] : translations ['ncurators'];
     var ch_plural = (max_search_channel == 1) ? translations ['nchannel'] : translations ['nchannels'];
     $("#result-summary .numbers").html ('<span id="curator-found">' + curator_count + '</span>' + cu_plural + '<span class="divider">|</span><span id="channel-found">' + max_search_channel + '</span>' + ch_plural);
     $("#channel-result-clip .clip-center").html (max_search_channel + ' ' + ch_plural);

     search_scrollbars();
     try { $("#results-slider .slider-vertical").slider ("value", "100"); } catch (error) {};

     var plural = (max_search_channel == 1) ? translations ['resfound1'] : translations ['resfound2'];
     $("#result-head").html ('<span class="amount">' + max_search_channel + '</span><span>' + plural + '</span>');

     if (max_search_channel == 0)
       $("#no-result").show();
     else
       $("#no-result").hide();
     ...
     // next_search_page ();
     search_infinite_timex = setInterval ("last_search_channel_in_view()", 200);
     });
...
function next_search_page()
  {
  if (highest_search_channel < max_search_channel)
    {
    var next = highest_search_channel + 1;
    var d = $.get ("/playerAPI/search?text=" + encodeURIComponent (last_search_term) + '&' + "start=" + next + '&' + "count=9" + rx(), function (data)
       {
       var blocks = data.split (/^--$/m);
       var scroll_offset = $("#results-slider .slider-vertical").slider ("value");
       append_search_channel_result (blocks[4]);
       search_clicks_init();
       search_scrollbars();
       // var scroll_offset = $("#results-slider .slider-vertical").slider ("value", scroll_offset);
       search_infinite_timex = setInterval ("last_search_channel_in_view()", 200);
       });
    }
  }

function append_search_channel_result (block)
  {
  var html = '';
  ...
  for (var i = 0; i < lines.length; i++)
       ...
       if (lines [i] != '')
         {
         highest_search_channel++;
         var fields = lines [i].split ('\t');
         var channel = line_to_channel (lines[i]);
         pool [channel ['id']] = channel;
         searchresults_stack [highest_search_channel] = channel;
         var ago = ageof (channel ['timestamp'], true);
         var plural = (channel ['count'] == 1) ? translations ['episode'] : translations ['episodes'];
         var curator = channel ['curator'];
         curator = curator.replace (/^YouTube\s+user:\s+/, '');
         html += '<li id="search-channel-' + highest_search_channel + '">';
  ...
  var current_html = $("#channel-result").html();
  $("#channel-result").html (current_html + html);

  /* for infinite scroll */
  $("#channel-result li").removeClass ("fin");
  $("#channel-result li:last").addClass ("fin");
  }

function search_clicks_init()
  {
...
var search_infinite_timex;

function last_search_channel_in_view()
  {
  if (thumbing == 'search')
    {
    if ($("#channel-result li.fin").offset().top < $(window).height())
      {
      clearInterval (search_infinite_timex);
      $("#channel-result li").removeClass ("fin");
      log ("EXPOSED! at " + highest_search_channel);
      next_search_page();
      }
    }
  else
...
{
$("#results-list").height() + parseInt ($("#results-list").css("top"))
416
$("#results-constrain").height()
416
$("#channel-result li:last").offset().top
546
...
function next_video_segment (program, segment_cursor)
  {
  var segments = program ['segments'];
  for (var i = segment_cursor + 1; i < segments.length; i++)
    {
    if (i in segments && segments [i]['type'] == 'video')
      return segments [i];
    }
...
function next_video_segment_to_preload()
  {
  var nvs = next_video_segment (get_current_program(), current_segment);
  if (nvs)
    return nvs;
  ...
  var program = programgrid [program_line [next_program]];

  return next_video_segment (program, 0);
  }
...
function store_play_yt (starting_at)
  ...
  if (clip ['type'] == '3')
    ...
    else if (clip ['clipped'] && clip ['start-time'] > 0)
      play_this_youtube_video (pid, current_segment, clip ['start-time']);
    else
      play_this_youtube_video (pid, current_segment, 0);
    }

  preload_any_titlecard_graphics();
  }
...
function progdrag_end (why)
  ...
  if (which_segment != current_segment)
    ...
    var starting_at = seconds_offset - segment ['starting'] + (clip ['clipped'] ? clip ['start-time'] : 0);
    if (clip ['clipped'])
      log ("start next clip at: " + starting_at + " (clip runs from " + clip ['start-time'] + " to " + clip ['end-time'] + ")");
    else
      log ("start next clip at: " + starting_at);
    store_play_yt (starting_at);
    }
  else
    {
    yt_mini_seek_by_percentage (pct_within);
    }
  }
...
function preload_youtube_videos()
  ...
  var next_clip;
  var nvs = next_video_segment_to_preload();
  if (nvs)
    next_clip = nvs ['clip'];
  ...
  if (next_clip ['type'] == '3')
    ...
    preload_one_audio (next_clip);
    }
  else
    {
    var next_url = next_clip ['url'];
...
function preload_next_yt (id)
  ...
  var segment = next_video_segment (program, 0);
  var clip = segment ['clip'];

  if (clip ['type'] == '3')
    {
    preload_one_audio (clip);
    return;
    }
  var next_url = clip ['url'];
  var next_video_id;
  if (next_url.match (/v=([^&]+)/))
  ...
  preload_one_youtube_video (next_video_id, 'next-channel', 0);
  }
...
function preload_one_youtube_video (next_video_id, why, start_point)
  ...
  for (var i = 1; i <= 3; i++)
    ...
    if (!ytmini_video_id[i])
      {
      log ('preloading episode: ' + next_video_id + " in slot: " + i + ", start point: " + start_point);
      $("#ym" + i).css ("zIndex", 25);
...
function update_yt_mini_progress_bar()
  ...
  /* Initially we have no duration of YouTube videos from Facebook, until we play them */
  if (program ['total-duration'] < 0 || !program ['duration'])
    {
    program ['total-duration'] = duration;
  ...
  if (duration && duration > 0 && duration != segment ['duration'] && segment ['type'] == 'video')
    {
    /* database is off, attempt to reflect reality but can't really do very much here */
    clip ['measured-duration'] = duration;
    if (segment ['duration'] > duration || clip ['clipped'] == false)
      {
      log ('duration fixup: was ' + segment ['duration'] + ', now ' + duration);
      segment ['duration'] = duration;
      if (clip ['clipped'] == false)
  ...
  var offset_inside_clip = offset;
  if (clip ['clipped'])
    offset_inside_clip = offset - clip ['start-time'];
  var adjusted_pct = (segment ['starting'] + offset_inside_clip) * 100 / program ['total-duration'];
  if (!quiet)
  ...
  if (clip ['clipped'] && offset >= clip ['end-time'])
    {
    log ("**************** reached end of clip time as specified by CMS. offset=" + offset + ", end-time: " + clip ['end-time']);
    flip_next_episode();
    }
  }
...
function yt_mini_seek_by_percentage (percent)
  ...
  var seekto = duration * percent / 100;
  log ("seekto :: duration=" + duration + " percent=" + percent + " seekto=" + seekto);

  try { ytmini [mini_player].seekTo (seekto); } catch (error) {};
  }
...
/* used only in case a Facebook login occurs */
var browse_accept_channel;
...
function curation (id)
  ...
  if (id in canonicalized_curators)
    set_hash ('#!' + id);
  else
    set_hash ('#!curator=' + id);
...
function browse()
  ...
  if (sphere != browse_sphere)
    {
    log ("region doesn't match browse cache! reloading");
    var d = $.get ("/playerAPI/category?lang=" + sphere + '&' + 'v=32' + rx(), function (data)
      {
      var lines = data.split ('\n');
      var fields = lines[0].split ('\t');
...
function autorotate_home()
  ...
  if (thumbing == 'home')
    ...
    if (++home_autorotate_count % 2 == 0)
      rotate_banner();
    home_timex = setTimeout ("autorotate_home()", 3000);
    }
  }
...
function signup_field_init (callback)
  ...
  $("#btn-home-sign-in-fb").click (redirect_to_facebook_login);
...
function redirect_to_facebook_login()
  {
  log ('redirect to facebook login! bye');
  setcookie ("fb-return-hash", location.hash);
  if (browse_accept_channel)
    setcookie ("fb-return-subscribe", browse_accept_channel);
