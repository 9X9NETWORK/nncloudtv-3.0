...
var ytmaxx = [];
var ytmaxx_video_id = [];
var ytmaxx_previous_state = [-2, -2, -2];
...
function init()
  ...
  $("#home").unbind();
  // $("#home").click (function() { player ("guide", "11"); });
  $("#home").click (home);
  $("#guide").unbind();
  $("#guide").click (guide);
  $("#browse").unbind();
...
function hash_changed()
  ...
  if (location.hash != current_hash)
    ...
    if (current_hash.match (/^#!(\d+)/))
      ...
      if (thumbing == 'player')
        {
        if (id in player_history ['player'])
          hash_changed_player (id);
        else
          hash_changed_store (id);
        }
      else if (thumbing == 'store' || thumbing == 'guide')
        {
        if (id in player_history ['player'] && ! (id in player_history ['store']))
          hash_changed_player (id);
        else
          hash_changed_store (id);
        }
...
function hash_changed_player (id)
  ...
  var cursor = first_position_with_this_id (id);
  if (thumbing == 'player')
    {
    if (cursor != 0)
      player_start_inner (cursor);
    }
  else
    {
    ipg_cursor = cursor;
...
function hash_changed_store (id)
  {
  log ('hash changed store');
  $("#setbubbleclick").hide();
  if (thumbing != 'store')
    {
    store_last_channel = id;
  ...
  if (id in player_history ['store'])
    {
    log ('[store hash set] known from history: ' + id);
    var stuff = player_history ['store'][id];
    store_recommend_index = stuff ['store_recommend_index'];
    log ('[store hash set] set: ' + stuff ['set'] + ' preview type: ' + stuff ['store_preview_type'] + ' store recommend index: ' + stuff ['store_recommend_index']);
    if (stuff ['store_preview_type'] == 'recommended')
      {
      switch_to_recommended_tab();
      /* trick recommended() into loading this channel */
      store_recommend_leftoff [stuff ['set']] = stuff ['store_index'];
      recommended (store_recommend_index);
      }
    else if (stuff ['store_preview_type'] == 'share')
      {
      /* turn off glowing */;
      store_recommend_off();
      }
    else if (stuff ['store_preview_type'] == 'category')
      {
      /* PDQ */
      switch_to_category_tab();
      var ugly = player_history ['category'][id];
      log ('fullcat: ' + ugly ['fullcat']);
      var catarrh = player_history ['catarrh'][ugly ['fullcat']];
      start_flipping_inner (ugly ['fullcat'], ugly ['shortcat'], catarrh ['data'], ugly ['which_channel']);
      }
    store_tab = stuff ['store_tab'];
    store_preview_type = stuff ['store_preview_type'];
    store_cat = stuff ['store_cat'];
    }
  ...
  if (lines.length > 0)
    ...
    for (var i = 2; i < lines.length; i++)
      ...
      if (lines [i] != '')
        ...
        var pinfo = { 'id': fields[1], 'channel': channel_id, 'type': fields[3], 'url1': 'fp:' + fields[8],
        ...
        if (pinfo ['submeta'].match (/:/))
          ...
          for (var m in submetas)
            ...
            if (typeof (submetas [m]) == 'string')
              ...
              log ('META');
              var kvs = {};
              var lines = submetas[m].split ('\n');
              for (j = 0; j < lines.length; j++)
                {
                if (lines[j].match (/:/))
                  {
                  var kv = (lines[j]).split (/:\s*/)
                  kvs [kv[0]] = kv[1];
                  log ('  ' + kv[0] + '=' + kv[1]);
                  }
                }
        ...
        if (pinfo ['url1'].match (/\|/))
          ...
          for (var j = 0; j < urls.length; j++)
            ...
            pcopy ['url1'] = canonicalize_youtube_url (url);
            pcopy ['name'] = names [j];
            pcopy ['desc'] = descs [j];
            pcopy ['thumb'] = thumbs [j]; 
            log ("SUBEPISODE PIECE: " + pcopy ['url1']);
            pcopy ['sort'] = ++count;
            if ((j+1) in begin_titles)
...
function custom_url (path)
  ...
  else if (path == 'view' || location.hash != '')
    {
    view_landing();
    return;
    }
...
function view_landing()
  ...
  log ("VIEW LANDING: " + channel_id + " " + episode_id);
...
function jumpstart_inner()
  ...
  if ('nature' in pool [jumpstart_channel])
...
function escape()
  ...
  if (layer && layer.css)
    ...
    log ("escape: layer not defined! thumbing is: " + thumbing);
...
function quicklogin (u)
  ...
  var d = $.get ("/playerAPI/quickLogin?token=" + u + rx(), function (data)
    ...
    if (fields[0] == '0')
      ...
      if (u.length > 75)
        {
        alert ("Facebook login seems to have failed?")
        return;
        }
      if (debug_mode)
...
function player (mode, cursor)
  ...
  $("#curator-photo").unbind();
  $("#curator-photo").click (visit_current_curator);

  $("#player-holder .favorite").unbind();
  $("#player-holder .favorite").click (favorite);
...
function column_buttons()
  ...
  $("#recommendation").click (player_recommended);
...
return;
  ...
  $("#btn-share").unbind();
  $("#btn-share").click (function (event) { event.stopPropagation(); share_dropdown(); });
  }
...
function share_dropdown()
  ...
  if ($("#share-dropdown").css ("display") == 'block')
    {
    close_all_dropdowns();
    return;
    }
  ...
  $("#share-dropdown li").removeClass ("on");
  $("#share-input .textfield").val (share_url());

  $("#share-dropdown").show();

  $("#share-dropdown li:nth-child(2)").unbind();
  $("#share-dropdown li:nth-child(2)").click (function (event)
    {
    event.stopPropagation();
    email_layer();
    close_all_dropdowns();
  ...
  $("#share-dropdown li:nth-child(3)").unbind();
  $("#share-dropdown li:nth-child(3)").click (function (event)
    {
    event.stopPropagation();
    switch_to_facebook();
    close_all_dropdowns();
  ...
  $("#share-input").unbind();
  $("#share-input").click (function (event) { event.stopPropagation(); });
...
function email_layer()
  ...
  $("#btn-email-cancel").unbind();
  $("#btn-email-cancel").click (function() { $("#email-layer").hide(); thumbing = 'player'; });
...
function email_recaptcha()
  ...
  var d = $.get (query, function (data)
    ...
    if (fields[0] == '0')
      ...
      $("#email-captcha").html ('<img src="' + image + '">');
      $("#btn-email-send").unbind();
      $("#btn-email-send").click (share_via_mail);
      $("#btn-email-send").removeClass ("disable");
      $("#btn-email-recaptcha").unbind();
      $("#btn-email-recaptcha").click (email_recaptcha);
...
function share_via_mail()
  ...
  var to = $("#email-input .textfield").val();
  var content = $("#msg-input .textfield").val();
  var captcha = $("#email-captcha-input .textfield").val();
...
function player_minimize()
  ...
  $("#btn-expand").show();
  $("#btn-expand").unbind();
  $("#btn-expand").click (player_fullscreen);
  }
...
function player_fullscreen()
  ...
  $("#btn-shrink").show();
  $("#btn-shrink").unbind();
  $("#btn-shrink").click (player_minimize);
  }
...
function flip_next_episode()
  ...
  if (any_lingering_end_titlecard())
    return;
...
function settings_change_password()
  ...
  $("#btn-change-return-password").click (settings_change_password_save);
  }
...
function footer_company()
  ...
  $("#company-layer").show();
  $("#hint-layer, #hint-bubble").hide();
  $("#btn-company-close").unbind();
  $("#btn-company-close").click (function() { $("#company-layer").hide(); store(); });
  $("#about").unbind();
  $("#about").click (about);
  $("#contact").unbind();
...
function about()
  {
  $("#company-tabs li").removeClass ("on");
  $("#about").addClass ("on");
  $("#company-holder .input-panel").hide();
  $("#about-panel").show();
  company_tab = 'about';
  if (! ($("#about-list").html().match (/^\s*$/)))
    {
    scrollbar ("#about-content", "#about-list", "#about-slider");
    return;
    }
  log ('retrieving: about');
  var query = '/playerAPI/staticContent?key=about&lang=' + language + rx();
  var d = $.get (query, function (data)
    {
    var lines = data.split ('\n');
    var fields = lines[0].split ('\t');
...
function contact()
  ...
  $("#contact-panel").show();
  company_tab = 'contact';
  if (! ($("#contact-list").html().match (/^\s*$/)))
    {
    scrollbar ("#contact-content", "#contact-list", "#contact-slider");
    return;
    }
  log ('retrieving: contact');
  var query = '/playerAPI/staticContent?key=contact&lang=' + language + rx();
  var d = $.get (query, function (data)
    {
    var lines = data.split ('\n');
    var fields = lines[0].split ('\t');
...
function legal()
  ...
  $("#legal-panel").show();
  company_tab = 'legal';
  if (! ($("#legal-list").html().match (/^\s*$/)))
    {
    scrollbar ("#legal-content", "#legal-list", "#legal-slider");
    return;
    }
  log ('retrieving: legal');
  var query = '/playerAPI/staticContent?key=legal&lang=' + language + rx();
  var d = $.get (query, function (data)
    {
    var lines = data.split ('\n');
    var fields = lines[0].split ('\t');
...
function store_yourown_submit()
  ...
  if (url == '' || url.match (/http:\/\/(www\.)?(youtube|facebook)\.com(\/user)?\/?$/i))
    {
...
function store_yourown_submit_inner (url)
  ...
  $.post ("/playerAPI/channelSubmit", serialized, function (data)
    ...
    after_add_triple_button (position);
    update_cart_bubble (channels_in_guide());
    });
  }
...
function play_titlecard (which, titlecard)
  ...
  function()
    ...
    if (current_episode_phase == 1)
      {
      /* begin-title has ended */
      current_episode_phase++;
      store_play_yt();
      }
    else if (current_episode_phase == 5)
      {
      /* end-title has ended */
      current_episode_phase++;
      flip_next_episode();
...
function any_lingering_end_titlecard()
  {
  log ('lingering? phase is: ' + current_episode_phase);
  var pid = program_line [program_cursor];
  var program = programgrid [pid];
  if ((current_episode_phase == 3 || current_episode_phase == 4) && 'end-title' in program)
    {
    log ('lingering end titlecard: playing that instead of next episode');
    store_play_yt();
    return true;
    }
  log ('no lingering end titlecard');
  return false;
  }

/* needed to track for title-begin and title-end */
var current_episode;
var current_episode_phase;
/*
   phases: 0: init,
           1: playing begin titlecard,
           2, begin titlecard ended,
           3: playing video, 
           4: video ended, 
           5: playing end titlecard,
           6, end titlecard ended,
           7: ended
*/


function store_play_yt()
  {
  ...
  log ('-- play -- episode: ' + pid);

  var program = programgrid [pid];

  if (current_episode_phase == 0 && 'begin-title' in program)
    {
    current_episode_phase = 1;
    play_titlecard ('begin', program ['begin-title']);
    return;
    }
  else if ((current_episode_phase == 3 || current_episode_phase == 4) && 'end-title' in program)
    {
    current_episode_phase = 5;
    play_titlecard ('end', program ['end-title']);
    return;
    }

  current_episode_phase = 3;
...
function play_this_youtube_video (pid)
  ...
  if (ytmini [mini_player] != undefined)
    ...
    if (!used_preload)
      ...
      set_ambient_volume();
      clearTimeout (awt_timex);
      awt_timex = setTimeout ('are_we_there_yet("' + thumbing + '","' + mini_player + '","' + video_id + '")', 6000);
...
function seriously_are_we_there_yet (mode, player_id, video_id)
  {
...
function browse_accept_success (position, channel_id)
  {
  redraw_subscribe();
  player_pop_message ("This channel has been added to your Guide.");
  browse_accept_callback();
  }
...
function unfollow (real_channel, callback)
  ...
  var d = $.get (cmd, function (data)
    ...
    redraw_subscribe();
    player_pop_message ("This channel has been removed from your Guide.");
...
function stop_all_other_players()
  ...
  if (current_tube != 'yt')
    {
    try { ytmaxx [maxx_player].stopVideo(); } catch (error) {};
    try { ytmaxx [maxx_player].clearVideo(); } catch (error) {};
    ytmaxx_video_id [maxx_player] = undefined;
    }
  ...
  for (var i = 1; i <= 3; i++)
    ...
    if (ytmaxx [i] && i != maxx_player)
      {
      try { ytmaxx [i].stopVideo(); } catch (error) {};
      try { ytmaxx [i].clearVideo(); } catch (error) {};
      ytmaxx_video_id [i] = undefined;
      }
    }
  }
...
function setup_yt()
  ...
  if (!ytmaxx [1])
    ...
    var url = "http://www.youtube.com/apiplayer?version=3&enablejsapi=1&playerapiid=myytplayer1";

    swfobject.embedSWF (url, "ytapiplayer1", "100%", "100%", "8", null, null, params, atts);

    var params = { allowScriptAccess: "always", wmode: wmode, disablekb: "1" };
    var atts = { id: "myytplayer2" };
    var url = "http://www.youtube.com/apiplayer?version=3&enablejsapi=1&playerapiid=myytplayer2";

    swfobject.embedSWF (url, "ytapiplayer2", "100%", "100%", "8", null, null, params, atts);

    var params = { allowScriptAccess: "always", wmode: wmode, disablekb: "1" };
    var atts = { id: "myytplayer3" };
    var url = "http://www.youtube.com/apiplayer?version=3&enablejsapi=1&playerapiid=myytplayer3";

    swfobject.embedSWF (url, "ytapiplayer3", "100%", "100%", "8", null, null, params, atts);

    setup_yt_timex = setTimeout ("setup_yt_timeout()", 30000);
    }
  else
    log ('setup_yt: was already set up!');
  }
...
function play_youtube (video_id)
  ...
  /* is this video preloaded already? */
  for (var i = 1; i <= 3; i++)
    {
    if (ytmaxx_video_id [i] == video_id)
      {
      log ('using preload at #' + i);
      maxx_player = i;
      if (thumbing != 'store' && thumbing != 'store-wait')
        {
        /* don't do this now if we have random starting points in the store */
        try { ytmaxx [i].seekTo (0); } catch (error) {};
        }
      try { ytmaxx [i].playVideo(); } catch (error) {};
      try { ytmaxx [i].unMute(); } catch (error) {};
      $("#yt" + i).css ("z-index", "3");
      used_preload++;
      }
    else
      {
      try { ytmaxx [i].mute(); } catch (error) {};
      try
        {
        if (ytmaxx [i].getPlayerState() != 2)
          ytmaxx [i].pauseVideo();
        }
      catch (error)
        {
  ...
  if (ytmaxx [1])
    {
    load_youtube (video_id, true);
    preload_next_youtube_episode();
    return;
    }
  else if (yt_kickstart)
    {
    log ('kickstart already in progress');
    return;
    }
  else
    {
    log ('play_youtube: youtube was not set up! doing that now. kickstart: ' + video_id);
    yt_kickstart = true;
    yt_kickstart_id = video_id;
    yt_kickstart_playflag = true;
    setup_yt();
    }
  }
...
function slot_status()
  {
  log ('slots: ' + ytmaxx_video_id [1] + ', ' + ytmaxx_video_id [2] + ', ' + ytmaxx_video_id [3]);
  }

function load_youtube (video_id, playflag)
  {
  log ('load yt: ' + video_id + ' playflag: ' + playflag);
  slot_status();

  if (yt_kickstart && yt_kickstart_id == video_id)
    {
    yt_kickstart = false;
    yt_kickstart_playflag = false;
    }
  else if (yt_kickstart)
    {
    log ('ATTN: kickstart in progress, kickstart id: ' + yt_kickstart_id + ', this id: ' + video_id);
    if (playflag)
      {
      log ('substituting this video id for existing kickstart');
      yt_kickstart_id = video_id;
      yt_kickstart_playflag = true;
      }
    return;
  ...
  if (ytmaxx [1])
    {
    log ('ytmaxx[1] at least, is ready');
    }
  else if (yt_kickstart)
    {
    /* notreached */
    ...
    {
    log ('load_youtube: youtube was not set up! doing that now. kickstart: ' + video_id);
    yt_kickstart = true;
    yt_kickstart_id = video_id;
    yt_kickstart_playflag = playflag;
    setup_yt();
    return;
    }
  ...
  for (var i = 1; i <= 3; i++)
    ...
    if (ytmaxx_video_id [i] == video_id)
      {
      log ('already running on player ' + i);
      return;
      }
    }
  ...
  if (slot)
    {
    try { ytmaxx [slot].loadVideoById (video_id, 0, yt_quality); } catch (error) {};
    if (playflag)
      {
      log ('playing ' + video_id + ' in slot ' + slot);
      $("#yt1, #yt2, #yt3").show();
      maxx_player = slot;
      try { ytmaxx [slot].playVideo(); } catch (error) {};
      try { ytmaxx [slot].unMute(); } catch (error) {};
      for (var i = 1; i <= 3; i++)
        {
        if (i != slot)
          $("#yt" + i).css ("z-index", "1");
      ...
      {
      try { ytmaxx [slot].pauseVideo(); } catch (error) {};
      try { ytmaxx [slot].mute(); } catch (error) {};
      }
    ytmaxx_video_id [slot] = video_id;
    }
...
function yt_confusion()
  {
  log ('yt_confusion: attempting to reload: ' + ytmaxx_video_id [maxx_player]);
  ytmaxx [maxx_player].loadVideoById (ytmaxx_video_id [maxx_player]);
  }
...
function physical_seek (offset)
  ...
  switch (tube())
    {
    case "yt":
      if (ytmaxx [maxx_player])
        try { ytmaxx [maxx_player].seekTo (offset); } catch (error) {};
      break;
    }
  }
...
function physical_offset()
  ...
  switch (tube())
    ...
    case "yt":
      ...
      if (ytmaxx [maxx_player] && ytmaxx [maxx_player].getCurrentTime)
        return ytmaxx [maxx_player].getCurrentTime();
      else
        return 0;
...
function physical_length()
  ...
  switch (tube())
    ...
    case "yt":
      ...
      var duration = 1;
      if (ytmaxx [maxx_player] && ytmaxx [maxx_player].getDuration)
        try { duration = ytmaxx [maxx_player].getDuration(); } catch (error) {};
      return duration;
...
function physical_is_paused()
  ...
  switch (tube())
    ...
    case "yt":
      ...
      if (ytmaxx [maxx_player])
        return ytmaxx [maxx_player].getPlayerState() == 2;
      else
        return false;
...
function physical_replay()
  ...
  switch (tube())
    ...
    case "yt":
      ...
      if (ytmaxx [maxx_player] && ytmaxx [maxx_player].seekTo)
        {
        ytmaxx [maxx_player].seekTo (0, false);
        ytmaxx [maxx_player].playVideo();
        }
      break;
...
function physical_volume()
  ...
  switch (tube())
    {
    case "yt":
      if (ytmaxx [maxx_player] && ytmaxx [maxx_player].getVolume)
        return ytmaxx [maxx_player].getVolume() / 100;
      else
        return 1;
    }
  }
...
function physical_set_volume (volume)
  ...
  switch (tube())
    ...
    case "yt":
      ...
      if (ytmaxx [maxx_player] && ytmaxx [maxx_player].setVolume)
        ytmaxx [maxx_player].setVolume (100 * volume);
...
function switch_to_facebook()
  ...
  if (FB)
    ...
    notice_ok (thumbing, 'Facebook error, plase try again later', "");
  }
...
function yt_player_state()
  ...
  try { yt_state = ytmaxx [maxx_player].getPlayerState(); } catch (error) {};
...
function play_slideshow()
  ...
  flowplayer ("audio" + audio_player, "http://releases.flowplayer.org/swf/flowplayer-3.2.7.swf",
    {
  ...
  $("#yt1, #yt2, #yt3, #fp1, #fp2").hide();
  $("#ss").show();

  current_tube = 'ss';

  $("#slide-1").fadeIn();
  flowplayer("audio" + audio_player).play();
...
function next_slide()
  ...
  if (thumbing != 'ipg' && thumbing != 'browse')
    ss_timex = setTimeout ("next_slide()", 7000);
  }
...
function curation (id)
  ...
  $("#curator-layer").removeClass("curator").removeClass("visitor");
  $("#curator-layer").addClass (showing).fadeIn(400);
  ...
  var d = $.get (query, function (data)
    ...
    for (i = 0; i < lines.length; i++)
      ...
      if (lines [i] != '')
        ...
        html += '<li id="curator-ch-' + ch_count + '">';
        html += '<p class="channel-title">' + channel ['name'] + '</p>';
        ...
        html += '<p class="default-channel-title">' + channel ['title'] + '</p>';
...
// $("#followings-wrap").css ("position", "relative"); /* temporary fix */
// $("#followings-wrap").css ("top", "20px"); /* temporary fix */
// $("#home-list").css ("overflow", "visible");
  setTimeout ('$("#banner").slideUp (400)', 30000);
  $("#closebtn").unbind();
  $("#closebtn").click (function() { $("#banner").slideUp (400); });
  home_trending();
  home_subscriptions();
  home_right_column();
  stop_all_players();
