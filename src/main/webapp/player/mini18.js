...
function set_language (lang)
  ...
  $("#selected-user .tail").html ("'s " + translations ['channels']) /* zh? */
...
function tvpreview_set_channel_inner (real_channel, episode_id, reportflag)
  ...
  if (episode_id != '')
    ...
    if (!program_cursor)
      ...
      message ('Cannot find this episode!', 6000, "switch_to_guide()");
      return;
      }
    }
  else
...
function switch_to_ipg_inner()
  ...
  $("#sync-layer, #player-layer, #store-layer, #help-layer").hide();
...
function update_metadata()
  ...
  if (parseInt (ipg_cursor) >= 11 & parseInt (ipg_cursor) <= 99)
    {
    var grid = grid_location (ipg_cursor);
    $("#channel-info .section-title").html ('<span>' + translations ['channel'] + '</span><span id="ch-index">' + grid + '</span>');
    }
  else
...
function diagnostics()
  ...
  $("#btn-help-close").click (function() { $("#help-layer").hide(); thumbing = diagnostics_saved_thumbing; });
  $("#help-tabs li").unbind();
