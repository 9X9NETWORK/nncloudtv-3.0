...
alter table poi_event add column type               smallint(6) DEFAULT '0';
...
CREATE TABLE `systag_map` (
  ...
  `timeend` smallint(6) NOT NULL,
  `timestart` smallint(6) NOT NULL,
  PRIMARY KEY (`id`),
...
alter table poi add column updateDate timestamp not null DEFAULT CURRENT_TIMESTAMP;
alter table poi drop column updateDate;
alter table poi add column updateDate timestamp not null DEFAULT CURRENT_TIMESTAMP;
